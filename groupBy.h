...
namespace details
{
    ...
    struct return_type
    {
      typedef typename return_type< decltype( &T::operator() )>::type type;
    };
...
namespace details
{
    ...
    {
        typedef std::map<
            typename details::return_type< T >::type,
...
namespace details
{
    ...
    void groupByImpl( Map & map, Iterator const current, F f )
    {
        map[ f( *current ) ].push_back( *current );
    }

    template< typename Map, typename Iterator, typename F, typename... Fs >
    void groupByImpl( Map & map, Iterator const current, F f, Fs... fs )
    {
        groupByImpl( map[ f( *current ) ], current, fs... );
    }
}
...
groupBy( Iterator begin, Iterator const end, const F& f, Fs... fs )
{
    ...
    {
        details::groupByImpl( result, begin, f, fs... );
    }
